...
def parsear_ingredientes(texto: str) -> list[dict]:
    ...
    start = next(
        (i for i, l in enumerate(lines) if _RGX_INGREDIENTES_HEADER.match(l)), None
    )
    if start is None:
        return []
    ingredientes = []
...
def extraer_nombre(texto: str) -> str:
    """Extrae el nombre de la receta del texto."""
...
def extraer_calorias(texto: str) -> int | str:
    """Extrae el número de calorías de la receta."""
...
def extraer_preparacion(texto: str) -> list[str]:
    ...
    lines = [l.strip() for l in texto.splitlines()]
    start = next(
        (i for i, l in enumerate(lines) if _RGX_PREPARACION_HEADER.match(l)), None
    )
    if start is None:
        return []
    pasos = []
...
def normalizar_receta_desde_texto(texto: str) -> dict:
    """Normaliza una receta desde texto plano a un diccionario estructurado."""
    return {
