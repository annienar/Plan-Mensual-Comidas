...
def test_ingredient_unit_validation():
    ...
    for unit in valid_units:
        ingredient = Ingredient(nombre="test", unidad = unit)
        assert ingredient.unidad == unit

    # Test invalid unit
    with pytest.raises(ValueError, match="unit must be one of"):
        Ingredient(nombre="test", unidad="invalid")
...
def test_ingredient_alternatives_validation():
    ...
    # Test invalid alternatives type
    with pytest.raises(ValueError, match="alternatives must be a list"):
        Ingredient(nombre="test", alternativas="invalid")

    # Test invalid alternative type
    with pytest.raises(ValueError, match="all alternatives must be strings"):
        Ingredient(nombre="test", alternativas=["valid", 123])
...
def test_ingredient_formatted_properties():
    ...
    # Test formatted_unit
    assert Ingredient(nombre="test", unit="").formatted_unit == ""
